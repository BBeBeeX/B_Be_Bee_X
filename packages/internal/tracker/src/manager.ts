...
export class TrackerManager {
  ...
  addAdapter(adapter: TrackerAdapter): void {
    if (this.adapters.has(adapter.getName())) {
      logger.warn(`[TrackerManager] Adapter "${adapter.getName()}" already exists. Replacing...`)
    }
    ...
    try {
      adapter.initialize()
      logger.info(`[TrackerManager] Initialized adapter: ${adapter.getName()}`)
    } catch (error) {
      logger.error(`[TrackerManager] Failed to initialize adapter "${adapter.getName()}":`, error)
    }
  }
  ...
  async track(code: TrackerMapper, properties?: Record<string, unknown>): Promise<void> {
    ...
    if (enabledAdapters.length === 0) {
      logger.warn("[TrackerManager] No enabled adapters found for tracking")
      return
    }
  ...
  async captureException(error: unknown, properties?: Record<string, unknown>): Promise<void> {
    ...
    if (enabledAdapters.length === 0) {
      logger.warn("[TrackerManager] No enabled adapters found for exception capture")
      return
    }
  ...
  async identify(payload: IdentifyPayload): Promise<void> {
    ...
    if (enabledAdapters.length === 0) {
      logger.warn("[TrackerManager] No enabled adapters found for identification")
      return
    }
  ...
  async setUserProperties(properties: Record<string, unknown>): Promise<void> {
    ...
    if (enabledAdapters.length === 0) {
      logger.warn("[TrackerManager] No enabled adapters found for setting user properties")
      return
    }
