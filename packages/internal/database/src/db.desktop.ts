...
export async function initializeDB() {
  ...
  db = drizzle(
    async (sql, params, method) => {
      ...
      // wa-sqlite 在并发执行时容易产生状态竞争，这里用串行锁保护查询过程。
      ...
      try {
        ...
        logger.error(`Error executing SQL: ${sql} with params:${params}`, error)
        return { rows: [] }
      } finally {
